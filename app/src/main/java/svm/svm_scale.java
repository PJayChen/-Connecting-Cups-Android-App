...
public class svm_scale
{
	...
	private BufferedReader rewind_AssetsFiles(BufferedReader fp, String filename) throws IOException
	{
		fp.close();
//		return new BufferedReader(new FileReader(filename));
	...
	private BufferedReader rewind_InternalStorage(BufferedReader fp, String filename) throws IOException
	{
		fp.close();
//		return new BufferedReader(new FileReader(filename));
	...
	private void run(String []argv) throws IOException
	{
		...
		try {
			fp = new BufferedReader(new InputStreamReader(context.openFileInput(data_filename)));
//			fp = new BufferedReader(new FileReader(data_filename));
		...
		if(restore_filename != null)
		{
			...
			try {
				...
				// Read read restore range file from Assets.
				fp_restore = new BufferedReader(new InputStreamReader(context.getAssets().open(restore_filename)));
//				fp_restore = new BufferedReader(new FileReader(restore_filename));
			...
			fp_restore = rewind_AssetsFiles(fp_restore, restore_filename);
		}
		...
		fp = rewind_InternalStorage(fp, data_filename);
		...
		if (scaled_filename != null) {			
			try {
//				fp_scaled = new BufferedReader(new InputStreamReader(context.getAssets().open(scaled_filename)));
				fp_scaled = new BufferedWriter(new OutputStreamWriter(context.openFileOutput(scaled_filename, Context.MODE_PRIVATE)));
			} catch(IOException e) {
	...
	public static void main(String argv[], Context ctxt) throws IOException
	{
		context = ctxt;
